"""
Charaideo July 2026 Flood Snapshot — Run Configuration

Prepares the Sentinel-1 flood-mapping pipeline for Charaideo district
using the same generalized methodology validated on Sivasagar and Jorhat.

==================================================
SENTINEL-1 CONSTELLATION STATUS (July 2026)
==================================================

Same as Jorhat (upper Assam region):
...
For Charaideo (Upper Assam, ~26.9–27.2°N, 94.7–95.4°E):
...
Charaideo district (formerly part of Sivasagar district) was among
the affected districts. Brahmaputra tributaries run through the district.
...
Event window:        2026-07-18 → 2026-08-05
  (Core flood period; wider than Jorhat to account for possible
   later flood propagation into Charaideo's river systems)

Alternative event:   2026-07-29 → 2026-08-05
  (If S1 imagery is sparse in the 18–25 July window)
...
- Parameters may need re-tuning for Charaideo's specific terrain/noise
"""
...
# ---------------------------------------------------------------------------
# Charaideo run configuration
# ---------------------------------------------------------------------------

CHARAIDEO_CONFIG = FloodPipelineConfig(
    district_id="charaideo",

    # --- Date windows (VALIDATE against actual S1 acquisitions) ---
    # Pre-event baseline: 2026-06-25 → 2026-07-06
    # Must contain at least one S1 DESCENDING+IW+VH acquisition over Charaideo
    pre_start="2026-06-25",
    pre_end="2026-07-06",

    # Event window: 2026-07-18 → 2026-08-05
    # Core flood period; wider than Jorhat to capture possible
    # later flood propagation into Charaideo's Brahmaputra tributaries
...
ALTERNATIVE_WINDOWS = {
    ...
    "tighter_event": {
        ...
        "event_end": "2026-08-05",
        "reason": "Late-July event window if flooding peaked later in Charaideo",
    },
    "extended_event": {
...
// ============================================================
// SENTINEL-1 AVAILABILITY DISCOVERY — CHARAIDEO, JULY 2026
// ============================================================
...
// --- Step 1: Define Charaideo AOI ---
// Option A: Use the actual district geometry (preferred)
//   Upload data/charaideo_gee.geojson to GEE Assets
//   var charaideo = ee.FeatureCollection('projects/YOUR_PROJECT/assets/charaideo_gee');
//   var aoi = charaideo.geometry();

// Option B: Bounding box approximation (for discovery only)
var aoi = ee.Geometry.Rectangle([94.73, 26.91, 95.37, 27.23]);

// --- Step 2: Define search windows ---
var preStart = '2026-06-25';
var preEnd   = '2026-07-06';
var evtStart = '2026-07-18';
var evtEnd   = '2026-08-05';
...
var wideEvt = s1.filterDate('2026-07-15', '2026-08-10');

print('=== WIDER PRE (Jun 10 -> Jul 6) ===');
print('Count:', widePre.size());

print('=== WIDER EVENT (Jul 15 -> Aug 10) ===');
...
Map.addLayer(aoi, {color: 'yellow'}, 'Charaideo AOI');
'''
...
// ============================================================
// CHARAIDEO FLOOD SNAPSHOT — EXECUTION SCRIPT
// ============================================================
...
// Upload data/charaideo_gee.geojson as GEE asset
var charaideo_fc = ee.FeatureCollection('projects/YOUR_PROJECT/assets/charaideo_gee');
var aoi = charaideo_fc.geometry();
...
Map.centerObject(aoi, 10);
Map.addLayer(aoi, {color: 'yellow'}, 'Charaideo District');
...
//   description: 'charaideo_flood_' + evtStart + '_' + evtEnd,
//   folder: 'ReliefOS_Flood_Snapshots',
//   fileNamePrefix: 'charaideo_flood_' + evtStart + '_' + evtEnd,
//   fileFormat: 'GeoJSON',
//   maxFeatures: 1e10
// });
'''
...
CHARAIDEO FLOOD SNAPSHOT — VALIDATION PROCEDURE
=================================================
...
F. VECTOR RESULT IS GEOGRAPHICALLY PLAUSIBLE
   -> Flood polygons should align with:
     ...
     - Low-lying areas in Charaideo district
...
J. CROSS-CHECK WITH CONTEXT
   ...
   -> Charaideo was among affected districts
...
def print_run_summary():
    """Print a summary of the Charaideo run configuration."""
    print("=" * 60)
    print("CHARAIDEO FLOOD SNAPSHOT — RUN CONFIGURATION")
    print("=" * 60)
    print()
    print("District: Charaideo, Assam, India")
    print(f"Export filename: {CHARAIDEO_CONFIG.generate_filename()}")
    print()
    print("Geometry source: PostGIS (export_district_geometry)")
    print(f"  District bounds: lon=[94.7346, 95.3717], lat=[26.9130, 27.2266]")
    print(f"  Vertices: 1,550")
    print()
    print("Date windows (VALIDATE AGAINST ACTUAL S1 DATA):")
    print(f"  Pre-event:  {CHARAIDEO_CONFIG.pre_start} -> {CHARAIDEO_CONFIG.pre_end}")
    print(f"  Event:      {CHARAIDEO_CONFIG.event_start} -> {CHARAIDEO_CONFIG.event_end}")
    print()
    print("Processing parameters:")
    print(f"  Collection:    {CHARAIDEO_CONFIG.collection}")
    print(f"  Polarization:  {CHARAIDEO_CONFIG.polarization}")
    print(f"  Instrument:    {CHARAIDEO_CONFIG.instrument_mode}")
    print(f"  Orbit:         {CHARAIDEO_CONFIG.orbit_pass}")
    print(f"  Threshold:     {CHARAIDEO_CONFIG.threshold_db} dB")
    print(f"  Speckle:       {CHARAIDEO_CONFIG.speckle_radius_m}m focal_median")
    print(f"  Connected:     {CHARAIDEO_CONFIG.connected_pixel_neighborhood} px neighborhood, gte {CHARAIDEO_CONFIG.min_connected_pixels}")
    print(f"  Vector scale:  {CHARAIDEO_CONFIG.vector_scale}m")
    print(f"  Area filter:   > {CHARAIDEO_CONFIG.area_threshold_m2} m\u00b2")
    print()
    ...
    print()
    print("Next steps:")
    print("  1. Upload data/charaideo_gee.geojson to GEE Assets")
